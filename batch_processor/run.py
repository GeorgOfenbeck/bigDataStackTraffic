...
def main():
    ...
    HDFS_NAMENODE = os.getenv("CORE_CONF_fs_defaultFS", "hdfs://localhost:8020")
    ...
    df = (
        ...
        .withColumn(
            "year", func.date_format("start_time", "yyyy").cast(tp.StringType())
        )
        .withColumn(
            "hours", func.date_format("start_time", "HH").cast(tp.IntegerType())
        )
        .withColumn(
            "part_of_day",
            func.when(func.col("hours").between(6, 23), "PART_1").otherwise("PART_2"),
        )
        .withColumn(
            "season",
            func.floor(func.col("month") % func.lit(12) / func.lit(3)) + func.lit(1),
        )
        .withColumn(
            "temperature",
            (func.col("Temperature(F)") - func.lit(32)) * func.lit(5) / func.lit(9),
        )
    )
    (
